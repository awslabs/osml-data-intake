...
export interface RegionConfig {
   ...
   */
  ecrCdkDeployRuntime: Runtime | null; // Runtime type from aws-cdk-lib/aws-lambda
  /**
...
/**
 * Configuration class for the Container Construct.
 */
export class ContainerConfig extends BaseConfig {
  ...
  /**
   * Creates an instance of ContainerConfig.
   * @param config - The configuration object for Container.
   */
...
/**
 * Represents the properties required to configure a container.
 *
 * @interface ContainerProps
 */
export interface ContainerProps {
  /**
   * The OSML account associated with the container.
   */
  account: OSMLAccount;
  ...
  /**
   * (Optional) Flag to instruct building the container from source.
   */
  buildFromSource?: boolean;

  /**
   * (Optional) Custom configuration for the Container Construct.
   */
...
export class Container extends Construct {
  ...
  /**
   * The repository access mode to assign when building containers for SageMaker.
   */
   ...
   * @param {ContainerProps} props - The properties of this construct.
  ...
  constructor(scope: Construct, id: string, props: ContainerProps) {
    ...
    this.config = props.config ?? new ContainerConfig();

    // Impute a regional config to use for the resources
    // Note: RegionalConfig.getConfig returns RegionalConfigData, not RegionConfig
    // For now, we'll create a simple region config
    const regionData = RegionalConfig.getConfig(props.account.region);
    this.regionConfig = {
      ecrCdkDeployRuntime: null, // Not used in this context
      maxVpcAzs: regionData.maxVpcAzs,
      sageMakerGpuEndpointInstanceType: "ml.p3.2xlarge",
      s3Endpoint: regionData.s3Endpoint
    };
